...
public class RTTMediaGrid : MonoBehaviour
{
    ...
    public void SetData(List<MediaVideoInfo> videos)
    {
        ...
        Debug.Log($"[RTTMediaGrid] SetData: {_allVideos.Count} items, columnsPerRow={_columnsPerRow}, visibleRowCount={_visibleRowCount}, itemsPerPage={itemsPerPage}, totalPages={TotalPages}, gridHeight={_height}, rowHeight={_rowHeight}");
    ...
    private void BindItemAtIndex(RTTMediaGridItem item, int index)
    {
        ...
        Debug.Log($"[RTTMediaGrid] BindItemAtIndex: index={index}, title={video.Title}, row={row}, col={col}, pos=({x:F0},{y:F0})");
